...
#动态时候使用的代码
class encoder(nn.Module):
    def __init__(self, init_adj, n_in, n_hid, n_out, n_time, do_prob, alpha):
        super(encoder, self).__init__()
        self.lstm = nn.LSTM(n_in, n_hid, batch_first=True)
        self.mlp = MLP(n_hid, 2 * n_hid, n_hid, do_prob)
        self.gat = GATLayer(n_time*n_hid, n_time*n_hid, do_prob, alpha)
        self.bn = nn.BatchNorm1d(n_hid)
        self.adj = nn.Parameter(init_adj.clone().detach())
        self.fc_mu = nn.Linear(n_hid, n_out)
        self.fc_std = nn.Linear(n_hid, n_out)
        # self.fc_mu1 = nn.Linear(n_hid, n_out)
        # self.fc_mu2 = nn.Linear(n_hid, n_out)
        # self.fc_std1 = nn.Linear(n_hid, n_out)
        # self.fc_std2 = nn.Linear(n_hid, n_out)
    ...
    def forward(self, inputs):
        ...
        # mu2 = self.fc_mu2(x)
        # mu: [num_sample 128, num_node*num_timepoint 180, num_out 1]
        # mu2 = mu2.reshape(inputs.size(0), inputs.size(1), inputs.size(2), -1)
        # mu: [num_sample 128, num_node 20, num_timepoint 9, num_out 1]
        std = self.fc_std(x)
        ...
        # std2 = self.fc_std2(x)
        # std: [num_sample 128, num_node*num_timepoint 180, num_out 1]
        # std2 = std2.reshape(inputs.size(0), inputs.size(1), inputs.size(2), -1)
        # std: [num_sample 128, num_node 20, num_timepoint 9, num_out 1]
        return mu, std
...
class decoder(nn.Module):
    def __init__(self, n_in, n_hid, n_time, n_node, do_prob, alpha):
        ...
        self.gat = GATLayer(n_hid*n_time, n_hid*n_time, do_prob, alpha)
        self.adj = nn.Parameter(torch.ones([n_node, n_node]))
        self.fc = nn.Linear(n_hid, n_in)
        self.bn = nn.BatchNorm1d(n_hid*n_time)
    ...
    def forward(self, inputs, idx):
        ...
        x = x.reshape(inputs.size(0), inputs.size(1), -1)
        # x: [num_sample 128, num_node 20, num_timepoint * num_feature 135]
        x = self.gat(x, self.adj)
        # x: [num_sample 128, num_node 20, num_hid 135]
        x = x[:, idx, :]
        # x: [num_sample 128, 1, num_hid 135]
        # x = self.batch_norm(x)
        # x: [num_sample 128, num_node 20, num_hid 135]
        #做一个特征融合
        # x = torch.bmm(torch.repeat_interleave(adj[[idx], :].clone().detach().unsqueeze(0), inputs.size(0), dim=0).cuda(), x)
        # x = torch.bmm(torch.repeat_interleave(self.adj[[idx], :].unsqueeze(0), inputs.size(0), dim=0), x)
        # x = torch.bmm(torch.repeat_interleave(self.add.unsqueeze(0), inputs.size(0), dim=0), x)
        # x: [num_sample 128, 1, num_hid 135]
        x = x.reshape(inputs.size(0), inputs.size(2), -1)
        # x: [num_sample 128, num_timepoint 9, num_hid 15]
        x = self.mlp2(x)  #这里不需要再做一次mlp
        # x: [num_sample 128, num_timepoint 9, num_hid 15]
        x = self.fc(x)
